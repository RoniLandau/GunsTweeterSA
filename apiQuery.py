...
c.execute("""CREATE TABLE tweets_16_22_geo_v2 (
            tweet_id text,
            text text,
            author_name text,
            author_id text,
...
def insert_tweet(twt, table_name):
    c.execute("INSERT INTO {} VALUES (?,?,?,?,?,?,?,?,0)".format(table_name), (
        twt.tweet_id, twt.text, twt.author_name, twt.author_id, twt.place_name, twt.geo,twt.place_id, twt.time_created))
...
def findPlaceName(index, place_id, include_places, dicti):
    ...
    if tupleData is None:
        ...
        geo = include_places[index]["geo"]
        # print(geo)

        geo = str(geo)
        # print(geo)
        # print("adding to place dict: " + place_id + " " + newname)
        dicti.update({place_id: (newname, geo)})
        return (newname,geo), isAdded
    return tupleData, isAdded
...
def parsePageResults(jsonResponse, usersDict, placesDict):
    ...
    for i in data:
        ...
        tuplePlaceData = tuplePlaceName[0]
        twt = Tweet(tweet_id, text, author_name, author_id, tuplePlaceData[0], tuplePlaceData[1], place_id, time_created)
        insert_tweet(twt, "tweets_16_22_geo_v2")
...
conn.request("GET", "/2/tweets/search/recent?query=(gun%20OR%20guns)%20("
                    "place_country%3AUS)&max_results=100&start_time=2022-07-16T14:00:00-04:00&end_time=2022-07-22T14"
                    ":00:00-04:00&tweet.fields=created_at&expansions=geo.place_id,"
                    "author_id&user.fields=created_at&place.fields=full_name,place_type,geo", payload, headers)
...
while metaSize > 3:
    conn.request("GET",
                 ...
                 f"place_country%3AUS)&max_results=100&start_time=2022-07-16T14:00:00-04:00&end_time=2022-07-22T14:00"
                 f":00-04:00&tweet.fields=created_at&expansions=geo.place_id,"
                 f"author_id&user.fields=created_at&place.fields=full_name,place_type,geo&next_token={next_token}",
                 payload, headers)
    res = conn.getresponse()
    data = res.read()
    x = data.decode("utf-8")
